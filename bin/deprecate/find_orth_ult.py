...
# sort qots
qotsrt = qot + '.srt'
os.system('export LC_ALL=C && sort --parallel=%s %s -o %s'%(cpu, qot, qotsrt))
...
###############################################################################
qipsrt = qip + '.srt'
os.system('export LC_ALL=C && sort --parallel=%s %s -o %s'%(cpu, qip, qipsrt))
...
###############################################################################
qcosrt = qco + '.srt'
os.system('export LC_ALL=C && sort --parallel=%s %s -o %s'%(cpu, qco, qcosrt))
...
for i in f:
    ...
    # get ip of ot
    st, ed, pairs = binary_search(Sips, qid, lambda x: x.split('\t', 2)[0])
    if not pairs:
        continue
    qips = [elem.split('\t')[1] for elem in pairs]
    st, ed, pairs = binary_search(Sips, sid, lambda x: x.split('\t', 2)[0])
    if not pairs:
        continue

    sips = [elem.split('\t')[1] for elem in pairs]
    visit = set()
    for qip in qips:
        for sip in sips:
